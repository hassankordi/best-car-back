const Transfer = require("../modal/transfer.modal")
const { StatusCodes } = require('http-status-codes')

//**  add item  **//
const addTransfer = async (req, res) => {    
// get the email from user token
let {nameOfBank} = req.body ;
let {email} = req.user ;
    ...
    try {
        const transfer = await Transfer.insertMany({img_src , nameOfBank})
        res.json({message:'تم اضافة الصوره بنجاح' , transfer})
    }
    catch (error) {
        // catch error 
